resource "aws_dynamodb_table" "lock_table" {
  name         = "${local.runner_prefix}-gh-actions-lock"
...
resource "aws_iam_policy" "dynamodb_policy" {
  name        = "${local.runner_prefix}-dynamodb-put-delete-policy"
...
data "external" "install_dependencies_global_lock" {
  program = ["bash", "${path.module}/scripts/requirements_clean_global_lock.sh", "/tmp/lambda_global_lock-${var.env}-${local.runner_prefix}/"]
}
...
data "archive_file" "lambda_zip_global_lock" {
  ...
  output_path = "/tmp/lambda_global_lock-${var.env}-${local.runner_prefix}-lambda_function.zip"
  depends_on  = [data.external.install_dependencies_global_lock]
}

resource "aws_lambda_function" "github_clean_global_lock_lambda" {
  function_name    = "${local.runner_prefix}-github-clean-global-lock"
  ...
  environment {
    variables = {
      DYNAMODB_TABLE              = "${local.runner_prefix}-gh-actions-lock"
...
resource "aws_cloudwatch_event_rule" "every_ten_minutes_global_lock" {
  name                = "${local.runner_prefix}-global-lock-every-ten-minutes-rule"
...
resource "aws_iam_role" "lambda_exec_global_lock" {
  name = "${local.runner_prefix}-global-lock-lambda-exec-role"
...
resource "aws_iam_policy" "lambda_policy_global_lock" {
  name        = "${local.runner_prefix}-global-lock-lambda-policy"
