locals {
  ...
  project_name                            = "${local.global_settings.locals.project_name}"
  product_name                            = "${local.global_settings.locals.product_name}"
  group_email                             = "${local.global_settings.locals.group_email}"
  team_name                               = "${local.global_settings.locals.team_name}"
  ...
  aws_profile_prefix                      = local.sl_aws_account_name
  git_org                                 = "${local.global_settings.locals.git_org}"

  # Sanitized values
  sanitized_project_name = replace(local.project_name, "_", "-")
  sanitized_git_org      = replace(local.git_org, "_", "-")
