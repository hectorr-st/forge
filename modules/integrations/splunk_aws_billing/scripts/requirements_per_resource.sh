...
HANDLER_FILE="${LAMBDA_DIR}/handler_per_resource.py"
...
if [[ ! -f "$HANDLER_FILE" ]]; then
    echo "{\"status\": \"error\", \"message\": \"Missing handler_per_resource.py in $LAMBDA_DIR\"}"
    exit 1
fi
...
cp "$HANDLER_FILE" .
zip -g "$ZIP_PATH" handler_per_resource.py >/dev/null
